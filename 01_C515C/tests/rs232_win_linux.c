...
Anpassung: CTS/RTS Flow Control aktiviert.
...
int OpenComport(int comport_number, char* portname, int baudrate)
{
  int baudr;
  ...
  Cport[comport_number] = open(portname, O_RDWR | O_NOCTTY | O_NDELAY);
  ...
  new_port_settings.c_cflag = baudr | CS8 | CLOCAL | CREAD | CNEW_RTSCTS;
...
int OpenComport(int comport_number, char* portname, int baudrate)
{
  ...
  Cport[comport_number] = CreateFileA(portname,
